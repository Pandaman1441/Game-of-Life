...
LifeSimulator::LifeSimulator(std::uint8_t sizeX, std::uint8_t sizeY)
{
    ...
    space.resize(sizeX, std::vector<bool>(sizeY, false));
}
...
void LifeSimulator::insertPattern(const Pattern& pattern, std::uint8_t startX, std::uint8_t startY)
{
    ...
    for (auto x = startX; x <= endX; ++x)
    {
        ...
        {
            space[x][y] = pattern.getCell(x - startX, y - startY);
        }
    }
}
...
void LifeSimulator::update()
{
    ...
    for (std::uint8_t x = 0; x < sizeX; ++x)
    {
        ...
        {
            std::uint8_t liveNeighborCount = neighbors(x, y);
    ...
    {
        space[coord.first][coord.second] = false;
    }

    // Revive cells
    for (auto coord : liveCoordinate)
    {
        space[coord.first][coord.second] = true;
    }
}

std::uint8_t LifeSimulator::neighbors(std::uint8_t x, std::uint8_t y)
{
    std::uint8_t count = 0;
...
{
    return static_cast<std::uint8_t>(space.size());
}

std::uint8_t LifeSimulator::getSizeY() const
{
    return static_cast<std::uint8_t>(space[0].size());
}
